...
export function generateLeafletMap(
  ...
  for (const [trackerId, rawPositions] of positionsByTracker) {
    ...
    trackerStats.push({ id: trackerId, color, pointCount: rawPositions.length, avgSpeed, duration, allCoords });
    trackerIdx++;
  }
...
geoData.engagement_lines.forEach(function(f) {
  if (!f.geometry) return;
  L.geoJSON(f, { style: { color: '#06b6d4', weight: 2, dashArray: '8,4', opacity: 0.8 } })
...
// 5. Drone tracks (glow + main per segment)
geoData.drone_tracks.forEach(function(f) {
  ...
  // Glow
  L.geoJSON(f, { style: { color: qColor, weight: 6, opacity: 0.15 } }).addTo(map);
  // Main line
  L.geoJSON(f, { style: { color: qColor, weight: 2.5, opacity: 0.85, dashArray: dash } })
