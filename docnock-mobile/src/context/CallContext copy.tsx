...
export const CallContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const { emitCancelCall, emitGetCallStartTime, emitLeaveCall, generateAgoraToken } =
    useCallSockets();
  ...
  const getCallPariticipantInfo = (participantId: string) => {
    socketInstance.emit(CallSocketEmitTypes.getCallParticipant,
      { participantId: participantId },
      (res: any) => {
        // console.log('get call participantInfo:', res, participantId);
        dispatch(setIsBusy(res))
      });
  }
const startCallTimer = (startTimeInSeconds: number) => {
  ...
  const toggleMic = async (params: mutePersonProps) => {
    setMicOn(pre => !pre);
    updateAudioMute(params)
    agoraEngine?.current?.muteLocalAudioStream(micOn);
  };
  ...
  const toggleVideo = async (params: muteVideoProps) => {
    setVideoOn(pre => !pre);
    updateVideoIcon(params)
    agoraEngine?.current?.muteLocalVideoStream(videoOn);
  };

  const toggleSpeaker = async () => {
    setOnSpeaker(pre => !pre);
    agoraEngine.current?.setEnableSpeakerphone(!onSpeaker);
  };
    ...
    try {
      ...
      if (agoraEngine?.current) {
        if (callRingerDetails?.callerId && callRingerDetails?.groupId) {
          if (callRingerDetails?.callerId === loginDetails?.profile?._id) {
            setAllCallDetails(pre => [
              ...(pre || []).filter(item => item.groupId !== callRingerDetails?.groupId),
            ]);
          }
          setAgoraToken(null)
          emitLeaveCall(
            ...
            callRingerDetails.isGroup ?? false,
